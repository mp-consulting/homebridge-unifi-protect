...
* Have you reviewed the [best practices documentation](https://github.com/mp-consulting/homebridge-unifi-protect/blob/main/docs/best-practices.md)?
...
#### Background
Read about [video autoconfiguration in HBUP](https://github.com/mp-consulting/homebridge-unifi-protect/blob/main/docs/autoconfiguration.md) for a primer on how stream selection, transcoding, and transmuxing works in HBUP.
  ...
  * UniFi Protect typically allows you to have up to three different RTSP streams available per camera (for most camera types). Each stream represents a different quality level - *High*, *Medium*, and *Low*, and you may choose to stream using any of them so long as they are enabled (HBUP [autoconfigures](https://github.com/mp-consulting/homebridge-unifi-protect/blob/main/docs/autoconfiguration.md) all the available RTSP streams, if it has the permissions to do so in Protect).
