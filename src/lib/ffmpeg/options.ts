...
export class FfmpegOptions {
    ...
    if(this.config.hardwareDecoding) {
      ...
      switch(this.codecSupport.hostSystem) {
        ...
        case 'raspbian':
          ...
          // Raspberry Pi. validateDecoder("h264_v4l2m2m", [ "yuv420p" ]);
          this.config.hardwareDecoding = false;
  ...
  private getHardwareTransferFilters(options: { hardwareDecoding?: boolean, hardwareTranscoding?: boolean }): string[] {
    ...
    //
    // 1. Software decode -> Software encode: No transfer needed, stay in software. 2. Software decode -> Hardware encode: Need hwupload to move data to GPU. 3.
    // Hardware decode -> Software encode: Need hwdownload to move data to CPU. 4. Hardware decode -> Hardware encode: No transfer needed, stay in hardware.
      ...
      switch(this.codecSupport.hostSystem) {
        ...
        case 'raspbian':
          ...
          decoderOptions = [
            ...
            // The decoder is broken in FFmpeg 7, unfortunately. "-codec:v", "h264_v4l2m2m"
          ];
    ...
    switch(this.codecSupport.hostSystem) {
      ...
      default:
        ...
        if(options.hardwareTranscoding) {
          ...
          //
          // format=same                   Set the output pixel format to the same as the input, since it's already in the GPU. w=...:h...                   
          // Scale the video to the size that's being requested while respecting aspect ratios.
          filters.push('vpp_qsv=' + [
  ...
  public streamEncoder(options: VideoEncoderOptions): string[] {
    ...
    //
    // crop                              Crop filter options, if requested. scale=...                         Scale the video to the size that's being requested
    // while respecting aspect ratios and ensuring our final dimensions are
    ...
    switch(this.codecSupport.hostSystem) {
      ...
      case 'macOS.Apple':
        ...
        //                               pipeline.
        encoderOptions.push('-codec:v', 'h264_videotoolbox', '-allow_sw', '1', '-realtime', '1', '-profile:v', this.getH264Profile(options.profile),
          '-level:v', '0', '-bf', '0', '-noautoscale', '-filter:v', videoFilters.join(', '), '-g:v', (options.fps * options.idrInterval).toString(),
          '-bufsize', (2 * options.bitrate).toString() + 'k', '-maxrate', adjustedMaxBitrate.toString() + 'k',
      ...
      case 'macOS.Intel':
        ...
        // -codec:v                      Specify the macOS hardware encoder, h264_videotoolbox.
        // -allow_sw 1                   Allow the use of the software encoder if the hardware encoder is occupied or unavailable.
        //                               This allows us to scale when we get multiple streaming requests simultaneously that can consume all the available
        // encode engines.
        // -realtime 1                   We prefer speed over quality - if the encoder has to make a choice, sacrifice one for the other.
        // -profile:v                    Use the H.264 profile that HomeKit is requesting when encoding.
        // -level:v 0                    We override what HomeKit requests for the H.264 profile level on macOS when we're using hardware-accelerated
        ...
        //                               of the bitrate on Intel-based Macs due to hardware / API limitations.
        // -g:v                          Set the group of pictures to the number of frames per second * the interval in between keyframes to ensure a solid
        //                               livestreaming experience.
        // -bufsize size                 This is the decoder buffer size, which drives the variability / quality of the output bitrate.
        // -maxrate bitrate              The maximum bitrate tolerance used in concert with -bufsize to constrain the maximum bitrate permitted.
        ...
        //                               pipeline.
        encoderOptions.push('-codec:v', 'h264_videotoolbox', '-allow_sw', '1', '-realtime', '1', '-profile:v', this.getH264Profile(options.profile),
          '-level:v', '0', '-bf', '0', '-noautoscale', '-filter:v', videoFilters.join(', '), '-b:v', options.bitrate.toString() + 'k',
          '-g:v', (options.fps * options.idrInterval).toString(), '-bufsize', (2 * options.bitrate).toString() + 'k',
          '-maxrate', adjustedMaxBitrate.toString() + 'k', ...((options.fps !== options.inputFps) ? [ '-r', options.fps.toString() ] : []));
      ...
      case 'raspbian':
        ...
        // -r framerate                  Set the output framerate. We use this to bypass doing this in filters so we can maximize the use of our hardware
        //                               pipeline.
        encoderOptions.push('-codec:v', 'h264_v4l2m2m', '-profile:v', this.getH264Profile(options.profile, true), '-bf', '0', '-noautoscale',
          '-reset_timestamps', '1', '-filter:v', videoFilters.join(', '), '-b:v', options.bitrate.toString() + 'k',
          '-g:v', (options.fps * options.idrInterval).toString(), '-bufsize', (2 * options.bitrate).toString() + 'k',
          '-maxrate', adjustedMaxBitrate.toString() + 'k', ...((options.fps !== options.inputFps) ? [ '-r', options.fps.toString() ] : []));
      ...
      default:
        ...
        // -r framerate                  Set the output framerate. We use this to bypass doing this in filters so we can maximize the use of our hardware
        //                               pipeline.
        encoderOptions.push('-codec:v', 'h264_qsv', '-profile:v', this.getH264Profile(options.profile), '-level:v', '0', '-bf', '0', '-noautoscale',
          '-filter:v', videoFilters.join(', '), '-g:v', (options.fps * options.idrInterval).toString(),
          '-bufsize', (2 * options.bitrate).toString() + 'k', '-maxrate', adjustedMaxBitrate.toString() + 'k',
          ...((options.fps !== options.inputFps) ? [ '-r', options.fps.toString() ] : []));
    ...
    if(this.config.hardwareTranscoding) {
      ...
      switch(this.codecSupport.hostSystem) {
        ...
        case 'raspbian':
          ...
          // For constrained environments like Raspberry Pi, when hardware transcoding has been selected for a camera, we limit the available source streams to
          // no more than 1080p. In practice, that means that devices like the G4 Pro can't use their highest quality stream for transcoding due to the
          // limitations of the Raspberry Pi GPU that cannot support higher pixel counts.
          return 1920 * 1080;
   ...
   * @param numeric      - Optional. If `true`, returns the numeric representation (e.g., "31"). If `false` or omitted, returns the standard string format
   *                       (e.g., "3.1").
   *
   ...
   * @param numeric - Optional. If `true`, returns the numeric representation (e.g., "100"). If `false` or omitted, returns the standard string format (e.g.,
   *                  "high").
   *
