...
export class ProtectTimeshiftBuffer extends EventEmitter {
  ...
  constructor(protectCamera: ProtectCamera) {
    ...
    // We use a small value for segment resolution in our timeshift buffer to ensure we provide an optimal timeshifting experience. It's a very small
    // amount of additional overhead for modern CPUs, but the result is a much better HKSV event recording experience.
  ...
  public async start(rtspEntry: RtspEntry): Promise<boolean> {
    ...
    // Ensure we have sane values configured for the segment resolution. We check this here instead of in the constructor because we may not have an
    // HKSV recording configuration available to us immediately upon startup.
    ...
    // Add the initialization segment to the beginning of the timeshift buffer, if we have it. If we don't, we're either starting up or something's
    // wrong with the API.
  ...
  public set configuredDuration(bufferMillis: number) {
    ...
    // Calculate how many segments we need to keep in order to have the appropriate number of seconds in our buffer. At a minimum we always want to
    // maintain a single segment in our buffer.
