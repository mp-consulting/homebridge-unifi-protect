...
export class ProtectRecordingDelegate implements CameraRecordingDelegate {
  ...
  public async updateRecordingActive(active: boolean): Promise<void> {
    ...
    if(!active) {
      ...
      // Turn off any potential inflight motion detection. Strictly speaking, this shouldn't be needed since any inflight motion sensor events will clear
      // themselves.
    ...
    // We're recording - update our recording state internally. We set this regardless of whether or not we succeed at starting the timeshift buffer in order
    // to keep our state consistent with HKSV.
    this._isRecording = active;
  ...
  public updateRecordingConfiguration(configuration: CameraRecordingConfiguration | undefined): void {
    ...
    // If we're set to an undefined state, it's because HomeKit can't figure out a valid configuration to use. This is typically due to a factory reset of
    // the camera or a similar edge case. We choose to handle it by stopping our timeshift buffer.
    if(!configuration) {
  ...
  public async *handleRecordingStreamRequest(): AsyncGenerator<RecordingPacket> {
    ...
    // If FFmpeg timed out it's typically due to issues with the video coming from the Protect controller. Restart the timeshift buffer to see if we can
    // improve things.
  ...
  private async startTransmitting(): Promise<boolean> {
    ...
    //
    // We want to keep feeding HomeKit until it tells us it's finished, or we decide we don't want to send anymore fMP4 packets. We treat this in a similar
    // way to how a DVR works where you can pause live television, but it continues to buffer what's being broadcast until you're ready to watch it. This is
    // the same idea.
    ...
    // Check to make sure something didn't go wrong when we start transmitting the stream. If there is, we're resetting our connectivity to the Protect
    // controller.
  ...
  private stopTransmitting(reason?: HDSProtocolSpecificErrorReason): void {
    ...
    if(this.ffmpegStream) {
      ...
      this.ffmpegStream.stop(
        (((reason !== undefined) && (reason !== HDSProtocolSpecificErrorReason.NORMAL)) || this.timeshift.isRestarting) ? false : undefined,
      );
      this.ffmpegStream = undefined;
    }
    ...
    // We actually have one less segment than we think we do since we counted the fMP4 stream header as well, which shouldn't count toward our total of
    // transmitted video segments.
