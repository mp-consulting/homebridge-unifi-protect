 ...
 *
 * protect-types.ts: Type definitions for the UniFi Protect API.
 */
/**
 * A semi-complete description of all the object types used by the UniFi Protect API.
 *
 * The UniFi Protect API is largely undocumented - these interfaces and types have been gleaned through a lot of experimentation and observation. Protect is
 * always evolving and I will attempt to keep up with the changes over time.
 *
 ...
 * `DeepPartial<...Config>` directly. All device interfaces include `[key: string]: ProtectKnownJsonValue` index signatures so that untyped API fields are
 * accessible without casting. Common device properties are inherited from `ProtectDeviceBaseInterface`.
 *
 * @module ProtectTypes
 */
/** @ignore */
...
/** @ignore */
export type ProtectKnownJsonValue = boolean | null | number | string | undefined | ProtectKnownJsonValue[] | {
...
export interface ProtectCameraConfigInterface extends ProtectDeviceBaseInterface {
  ...
  featureFlags: {
    ...
    videoModes: string[];
    zoom: ProtectPtzAxisRangeInterface & {
      ratio: number;
    };
  };
  fingerprintSettings: {
  ...
  smartDetectLines: [];
  smartDetectLoiterZones: (ProtectSmartZoneInterface & {
    loiterTriggers: {
      loiterTriggerTime: number;
      objectType: string;
    }[];
    triggerAccessTypes: string[];
  })[];
  smartDetectSettings: {
    audioTypes: string[];
  ...
  smartDetectZones: (ProtectSmartZoneInterface & {
    objectTypes: string[];
  })[];
  speakerSettings: {
    areSystemSoundsEnabled: boolean;
