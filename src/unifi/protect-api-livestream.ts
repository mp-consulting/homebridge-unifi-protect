...
const VALID_LIVE_FRAMES = new Set(Object.values(ProtectLiveFrame).filter((value) => typeof value === 'number'));
...
export class ProtectLivestream extends EventEmitter {
   ...
   * @param channel          - Camera channel to use, indexing the channels array in the camera's {@link ProtectTypes.ProtectCameraConfigInterface |
   *                           ProtectCameraConfig}.
  ...
  private async launchLivestream(cameraId: string, channel: number, options: LivestreamOptions): Promise<boolean> {
    ...
    try {
      ...
      ws.once('open', () => {
        ...
        this.heartbeat = setInterval(() => {
          ...
          if((Date.now() - this.lastMessage) > PROTECT_LIVESTREAM_HEARTBEAT_TIMEOUT) {
            ...
            if((this.ws?.readyState === WebSocketClient.CLOSING) || (this.ws?.readyState === WebSocketClient.OPEN)) {
      ...
      ws.once('error', (error: NodeJS.ErrnoException) => {
  ...
  private processLivestream(): void {
    ...
    this.ws.on('message', (data: Buffer | string) => {
      ...
      // Normalize the message into a Buffer so we can process it.
