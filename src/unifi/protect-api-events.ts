...
async function decodeFrame(log: ProtectLogging, packet: Buffer, packetType: ProtectEventPacketType): Promise<unknown> {
  ...
  if(frameType === ProtectEventPacketType.ACTION) {
    ...
    return (payloadFormat === EventPayloadType.JSON) ? JSON.parse(payload.toString()) as unknown : null;
  }
