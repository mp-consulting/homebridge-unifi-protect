...
export class ProtectStreamingDelegate implements HomebridgeStreamingDelegate {
  ...
  private ongoingSessions: Record<string, OngoingSessionEntry>;
  ...
  constructor(protectCamera: ProtectCamera, resolutions: [number, number, number][]) {
    ...
    if(!this.protectCamera.hints.hardwareTranscoding && !this.protectCamera.accessory.context.packageCamera &&
      ...
      this.log.warn('macOS detected: consider enabling hardware acceleration (located under the video feature options ' +
        'section in the HBUP webUI) for even better performance and an improved user experience.');
    }
    ...
    const options: CameraControllerOptions = {
        ...
        options: {
          ...
          video: {
            ...
            parameters: {
              ...
              // Through admittedly anecdotal testing on various G3 and G4 models, UniFi Protect seems to support only
              // the H.264 Main profile, though it does support various H.264 levels, ranging from Level 3 through
              // Level 5.1 (G4 Pro at maximum resolution). However, HomeKit only supports Level 3.1, 3.2, and 4.0
              // currently.
      ...
      streamingOptions: {
        ...
        audio: {
          ...
          codecs: [

            {
              ...
              // Protect doorbells and the Opus audio track over RTSP both use a 48 kHz audio sampling rate, which
              // HomeKit doesn't support. Since both 16 and 24 will divide cleanly into 48, we allow HomeKit to choose
              // it's preference. Otherwise, the livestream API uses a 16 kHz sampling rate.
        ...
        video: {
          ...
          // Retrieve the list of supported resolutions from the camera and apply our best guesses for how to map
          // specific resolutions to the available RTSP streams on a camera. Unfortunately, this creates challenges
          // in doing on-the-fly RTSP changes in UniFi Protect. Once the list of supported resolutions is set here,
          // there's no going back unless a user retarts HBUP. Homebridge doesn't have a way to dynamically adjust
          // the list of supported resolutions at this time.
          resolutions: resolutions,
        },
      },
    };
  ...
  public async prepareStream(request: PrepareStreamRequest, callback: PrepareStreamCallback): Promise<void> {
    ...
    if(!hasAudioSupport) {
      ...
      this.log.info('Audio support disabled.%s',
    ...
    // Prepare the response stream. Here's where we figure out if we're doing two-way audio or not. For two-way audio, we need to use a demuxer to separate
    // RTP and RTCP packets. For traditional video/audio streaming, we want to keep it simple and don't use a demuxer.
  ...
  private async startStream(request: StartStreamRequest, callback: StreamRequestCallback): Promise<void> {
    ...
    //   3. We are on a high latency streaming session (e.g. cellular). If we're high latency, we'll transcode by default unless the user has asked us
    //      not to. Why? It generally results in a speedier experience, at the expense of some stream quality (HomeKit tends to request far lower bitrates
    //      than Protect is capable of producing).
    //   4. The codec in use on the Protect camera isn't H.264.
    //
    // How do we determine if we're a high latency connection? We look at the RTP packet time of the audio packet time for a hint. HomeKit uses values of
    // 20, 30, 40, and 60ms. We make an assumption, validated by lots of real-world testing, that when we see 60ms used by HomeKit, it's a high latency
    // connection and act accordingly.
  ...
  private buildVideoArgs(request: StartStreamRequest, sessionInfo: SessionInfo, rtspEntry: RtspEntry, isTranscoding: boolean, isHighLatency: boolean,
    ...
    // -fflags flags                    Set the format flags to discard any corrupt packets rather than exit, generate a presentation timestamp if
    //                                  it's missing, ignore the decoding timestamp, and minimize buffering as well as latency. Adjusting timestamps
    //                                  is a necessity if we want to ensure we keep audio and video in sync, especially when using hardware
    //                                  acceleration.
    ...
    // -packetsize 1200                 Use a packetsize of 1200 for compatibility across network environments. This should be no more than HomeKit
    //                                  livestreaming MTU (1378 for IPv4 and 1228 for IPv6).
  ...
  // Construct FFmpeg audio arguments (codec, sampling rate, filters, and audio SRTP output).
  private buildAudioArgs(request: StartStreamRequest, sessionInfo: SessionInfo, useTsb: boolean | null | undefined): string[] {
    ...
    // -flags +global_header            Sets the global header in the bitstream. Needed for FDK-AAC to correctly initialize. For encoders like
    //                                  aac_at it becomes a no-op.
    ...
    const audioArgs = [
      ...
      // Take advantage of the higher fidelity potentially available to us from the Opus track in the RTSP stream. The livestream API only provides
      // an AAC track.
    ...
    if(afOptions.length) {
      ...
      audioArgs.push('-filter:a', afOptions.join(', '));
    }
    ...
    // -f rtp                           Specify that we're using the RTP protocol.
    // -flush_packets 1                 Ensure we flush our write buffer after each muxed packet.
    // -packetsize 384                  Use a packetsize as a multiple of the sample rate. HomeKit livestreaming wants a block size of 480 samples
    //                                  when using AAC-ELD. We loosely interpret that to mean less than 480 bytes per packet and use 384 since it's
    //                                  divisible by 16 kHz and 24 kHz.
    // -srtp_out_suite enc              Specify the output encryption encoding suites.
    // -srtp_out_params params          Specify the output encoding parameters. This is negotiated by HomeKit.
    audioArgs.push(
  ...
  private async startFfmpegProcess(request: StartStreamRequest, sessionInfo: SessionInfo, rtspEntry: RtspEntry, ffmpegArgs: string[],
    ...
    callback: StreamRequestCallback): Promise<Nullable<FfmpegStreamingProcess>> {
    ...
    if(useTsb) {
      ...
      const processSegmentQueue = createSegmentQueueProcessor(() => ffmpegStream.stdin ?? null);
