...
export class ProtectCamera extends ProtectDevice {
  ...
  private static readonly NIGHT_VISION_MAP: ReadonlyMap<string, number> = new Map([
  ...
  private static readonly NIGHT_VISION_BRIGHTNESS_MAP: ReadonlyMap<number, string> = new Map([
  ...
  private static readonly UFP_RECORDING_SWITCHES = [
  ...
  private configureMotionSmartSensor(): boolean {
    ...
    this.detectLicensePlate = this.getFeatureValue('Motion.SmartDetect.ObjectSensors.LicensePlate')
    ...
    if(this.ufp.featureFlags.smartDetectTypes.includes('licensePlate')) {
      ...
      if(enabledContactSensors.length) {
        ...
        this.log.info('Smart motion license plate contact sensor%s enabled: %s.',
  ...
  private async configureVideoStream(): Promise<boolean> {
    ...
    const cameraUrl = 'rtsps://' +
      (this.nvr.config.overrideAddress ?? this.ufp.connectionHost ?? this.nvr.ufp.host) + ':' + this.nvr.ufp.ports.rtsps.toString() + '/';
    ...
    if(!this.hints.recordingDefault) {
      ...
      switch(this.platform.codecSupport.hostSystem) {
        ...
        case 'raspbian':
          ...
          this.hints.recordingDefault =
            (this.findRtsp(1920, 1080, { maxPixels: this.stream.ffmpegOptions.hostSystemMaxPixels })?.channel.name ?? undefined) as string;
  ...
  private configureNightVisionDimmer(): boolean {
    ...
    service.getCharacteristic(this.hap.Characteristic.Brightness).onSet(async (value: CharacteristicValue) => {
      ...
      let nightvision = {};
      const fixedMode = ProtectCamera.NIGHT_VISION_BRIGHTNESS_MAP.get(level);
  ...
  private configureNvrRecordingSwitch(): boolean {
    ...
    for(const ufpRecordingSwitchType of ProtectCamera.UFP_RECORDING_SWITCHES) {
      ...
      service.getCharacteristic(this.hap.Characteristic.On).onSet(async (value: CharacteristicValue) => {
        ...
        // We only want to do something if we're being activated. Turning off the switch would really be an undefined state given that there are three
        // different settings one can choose from. Instead, we do nothing and leave it to the user to choose what state they really want to set.
        if(!value) {
        ...
        for(const otherUfpSwitch of ProtectCamera.UFP_RECORDING_SWITCHES) {
  ...
  public updateDevice(): boolean {
    ...
    if(this.hints.ledStatus) {
      ...
      this.accessory.getService(this.hap.Service.CameraOperatingMode)
        ?.updateCharacteristic(this.hap.Characteristic.CameraOperatingModeIndicator, this.statusLed);
    }
  ...
  private get nightVisionBrightness(): number {
    ...
    const mode = this.ufp.ispSettings.irLedMode;

    // Check the fixed mode-to-brightness mapping first.
    const brightness = ProtectCamera.NIGHT_VISION_MAP.get(mode);
    ...
    this.log.error('Unknown night vision value detected: %s.', mode);
