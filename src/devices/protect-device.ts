...
export abstract class ProtectDevice extends ProtectBase {
  ...
  protected configureMotionSensor(isEnabled = true, isInitialized = false): boolean {
    ...
    // Have we previously initialized this sensor? We assume not by default, but this allows for scenarios where you may be dynamically
    // reconfiguring a sensor at runtime (e.g. UniFi sensors can be reconfigured for various sensor modes in realtime).
    if(!isInitialized) {
  ...
  // Return a unique identifier for a Protect device. We need this for package cameras in particular, since they present multiple cameras in a
