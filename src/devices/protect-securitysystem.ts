...
export class ProtectSecuritySystem extends ProtectBase {
  ...
  private configureInfo(): boolean {
    ...
    this.accessory.getService(this.hap.Service.AccessoryInformation)?.updateCharacteristic(this.hap.Characteristic.Manufacturer, 'github.com/mp-consulting');

    // Update the model information for this security system.
    this.accessory.getService(this.hap.Service.AccessoryInformation)
      ?.updateCharacteristic(this.hap.Characteristic.Model, 'UniFi Protect Liveview Security System');

    // Update the serial number for this security system - we base this off of the NVR.
    this.accessory.getService(this.hap.Service.AccessoryInformation)
      ?.updateCharacteristic(this.hap.Characteristic.SerialNumber, this.nvr.ufp.mac + '.Security');

    // Update the hardware revision for this security system - we base this off of the NVR.
    this.accessory.getService(this.hap.Service.AccessoryInformation)
      ?.updateCharacteristic(this.hap.Characteristic.HardwareRevision, this.nvr.ufp.hardwareRevision);
  ...
  private setSecurityState(value: CharacteristicValue): void {
    ...
    // We have three different states which can be triggered (aside from disarming). Those states are home, away, and night. We use this as a
    // convenient way to easily enable or disable motion detection on a Protect controller and effectively give us scene-type functionality
    // in a nice way.
    switch(value) {
    ...
    for(const targetAccessory of this.platform.accessories) {
      ...
      // If we're disarming, then all Protect cameras will disable motion detection in HomeKit. Otherwise, check to see if this is one of the
      // cameras we want to turn on motion detection for.
      if(((newState !== SecuritySystemCurrentState.DISARMED) || targetCameraIds.length) &&
        targetCameraIds.some(thisCameraId => thisCameraId === targetUfp.id)) {
